...
- `writing-artifacts/roadmap.md` (chapter-layer alternation and pacing)

Drafting is not complete until the chapter satisfies both the roadmap rules and this protocol.
...
- Avoid repeating any sentence opening pattern more than twice in a row within a chapter.
- Avoid repeating a non-essential verb phrase more than three times across neighboring paragraphs.
- Avoid repeating a key metaphor family across two adjacent chapters unless the repeat is intentional and tied to progression (e.g., the same motif under changing stakes).

### Allowed exception pattern

- A repeated phrase may appear only when it is the artifact of character voice or memory residue and the repetition is meaning-bearing (for example, recurring code words, ritual language, or drift symptoms).

## Editorial anti-patterns to eliminate

- **Techno-babble as authority**: do not present concept mechanics as “officially simple.” Explain through observation, consequence, and one human response.
- **Information burst paragraphs**: no dense exposition blocks. Information must be attached to concrete scene stakes.
- **Emotionally flat summaries**: avoid telling the reader a character “understood everything”; demonstrate that understanding through a specific, irreversible choice.
- **Hyperactive scene transitions**: transitions must feel motivated by memory, interruption, or risk, not by paragraph management.

## Lattice vs Recording chapter tone rules
...
Use when lead layer is Present-Lattice.

- Preferred register: conceptual, architectural, social, and behavioral.
...
Use when lead layer is Recording.
...
- Follow the chapter-level lead-layer order in `writing-artifacts/roadmap.md` by default.
- If a chapter departs from the alternation matrix, log a reason in local draft notes and the corresponding chapter postmortem.
- Lattice leads must begin with a present-tense social/structural pressure scene unless explicitly marked as single-layer.
- Recording leads should begin with an embodied entry point (location, body state, or environmental detail) before analysis.
- Between chapter types, maintain a perceivable tonal contrast: abstraction in the Lattice, embodied specificity in recordings.

## Tone progression by arc context

### Act 1
- Curiosity-led restraint, controlled emotional opening.
- Introduce stakes by showing what is hidden, not by explaining the hidden policy directly.

### Act 2
- Increase fatigue and behavioral asymmetry.
- Keep revelations measured; let readers sense instability before each explicit naming.

### Act 3
- Increase inevitability and cost density.
- Increase sentence precision around moral and technical choices.

### Act 4
- Keep prose calm enough to hold contradictory outcomes at once.
- The final truth should be held in concrete action, with minimal authorial interpretation.

## Draft compliance checklist (for each chapter)

Before considering a chapter draft complete, run this list and record any exception:
...
7. Do scene transitions emerge from character pressure, risk, or memory residue, not list-like plotting?
