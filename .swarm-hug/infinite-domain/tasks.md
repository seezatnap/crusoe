...
- [A] (#4) Build the setting and mechanics bible for the Lattice, Archive, drift, noise, mind-jump bleed, and transition-era context, including what is explicit vs forbidden lore [5 pts] (blocked by #3)
...
- [A] (#15) Build a Rust orchestration binary that runs #12–#14 plus basic length checks in one command and writes signed artifacts per chapter to `writing-artifacts/quality/` [5 pts] (blocked by #12, #13, #14)
...
## Follow-up tasks (from sprint review)
- [B] (#40) Update `cliche-detection`, `voice-consistency`, and `continuity-check` to use `AnalysisInput::resolve_target_path()` for validation, file reads, and report `target` paths so `with_working_directory(...)` and relative targets behave consistently (blocked by #39)
- [B] (#41) Add regression tests for all three analyzer crates (`cliche-detection`, `voice-consistency`, `continuity-check`) that use a non-root `working_directory` with relative targets to confirm path resolution is applied end-to-end, including report metadata and file access (blocked by #39)
