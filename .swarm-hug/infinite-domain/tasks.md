...
- [A] (#22) Draft `chapters/chapter-06.md` in the 4,000–6,000-word range: expand Juno’s fixation as Sable appears in background details, and increase subtle bleed symptoms in present scenes [5 pts] (blocked by #21)
...
## Follow-up tasks (from sprint review)
- [A] (#51) Update `writing-artifacts/roadmap.md` chapter tracker status for chapters 01–05 so it reflects completed draft state (and current word counts) instead of all entries remaining “Not started,” now that the drafts are in place. (blocked by #21)
- [B] (#52) Resolve remaining `POV-MARKER-001`/voice-consistency metadata gaps in `chapters/chapter-05.md` (either by adding supported POV/dialogue markers or adjusting parser expectations) and rerun `scripts/run-quality-session.sh` to capture a cleaner `writing-artifacts/quality/chapter-05/quality-orchestrator-*` report.
